/**
 * Référence des départements français (code + nom) pour le menu de livraison du panier.
 *
 * ⚠️ Donnée de RÉFÉRENCE, pas de tarif : les noms sont fixes (ils ne changent jamais),
 * au même titre que la correspondance des normes. Les TARIFS de port, eux, restent lus
 * dans `tables.json` (généré depuis l'Excel) — ce fichier ne contient aucun prix.
 *
 * Périmètre de livraison : France métropolitaine + Corse UNIQUEMENT (décision PA — pas
 * de DROM/outre-mer). Ordre : 01→95, Corse 2A/2B intercalée après 19.
 * Tous ces départements sont tarifés dans `tables.expedition` → le port est toujours
 * calculé (jamais « sur devis » pour un choix du menu).
 */

export interface Departement {
  /** Code tel qu'attendu par le moteur de port (clé de `tables.expedition`). */
  code: string;
