...
|---|---|---|---|
| **B1 — Moteur de prix** | Réplique des tables Excel + tests | 🔄 Spécifié + 3 skills + **T1→T5 faits** (export + `types.ts` + `lookups.ts` + aiguillage + `engine.ts` + `shipping.ts` ; Vitest 45 tests, relus) — reste T6 (poids) → T9 | — (constructible) |
...
✅ **T5 fait** : `shipping.ts` (port par département + franco lu des données ; dépt inconnu → sur devis), 11 tests, relu ✅. **En attente de commit (accord PA).**
➡️ **Prochaine étape = T6** : `weight.ts` — poids total = poids média (`tables.poids`, kg/m²) × surface × quantité. Puis T7 point d'entrée `calculerPrix()` (assemble prix + port + poids ; traduire `portSurDevis`), **T8/T9 vecteurs dorés** (⚠️ y solder les 2 dettes : preuve « Excel = moteur » sur B→F + cas **hors-format** surface > 50 dm²).
