...
|---|---|---|---|
| **B1 — Moteur de prix** | Réplique des tables Excel + tests | 🔄 Spécifié + 3 skills + **T1→T7 faits** (export + contrat + lookups + aiguillage + engine + shipping + point d'entrée ; T6 abandonné ; Vitest **53 tests**, tout relu reviewer + validator) — **moteur complet** ; reste **T8/T9** (vecteurs dorés « Excel = moteur ») | — (constructible) |
...
✅ **T7 fait** : `index.ts` — `calculerPrix()` assemble prix + port, 8 tests (53 au total), relu reviewer ✅ + validator CONFORME. **Moteur complet de bout en bout.** **En attente de commit (accord PA).**
➡️ **Prochaine étape = T8/T9 — vecteurs dorés** : générer des cas depuis l'Excel Netair (onglet « Devis interne » : PRU/PTU) et prouver « Excel = moteur » au centime sur **toutes** les gammes/classes/paliers, + **cas limites** (⚠️ y solder les 2 dettes : preuve B→F + **hors-format** surface > 50 dm²). Déclenche `netair-pricing-qa`. **Indispensable avant toute mise en ligne.**
