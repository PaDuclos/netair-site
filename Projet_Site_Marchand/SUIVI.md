...
|---|---|---|---|
| **B1 — Moteur de prix** | Réplique des tables Excel + tests | 🔄 Spécifié + 3 skills + **T1→T4·A faits** (export + `types.ts` + `lookups.ts` + aiguillage des gammes ; Vitest 22 tests, relus) — reste T4·B (engine) → T9 | — (constructible) |
...
✅ **T4 Étape A faite** : aiguillage des gammes par méthode (champ `methode` lu d'`Infos_Netair`, rien en dur, 34 gammes, sécurité sur_devis + avertissement), relu (réserves traitées), 22 tests verts.
➡️ **Prochaine étape = T4 Étape B** : `engine.ts` — l'**aiguilleur** (lit `gamme.methode`) + les **6 méthodes A→F** (assemblage du coût via `lookups.ts`) + **prix = coût × ratio** (arrondi 2 déc.) + statuts/`DetailCalcul`. Recoupé avec `cost_calculator.py`/Excel, tests « Excel = moteur ». ⚠️ Sort des prix → **`netair-pricing-validator`** en plus du reviewer. _(À unifier en Étape B : type `MethodeCalcul` partagé entre `types.ts` et `lookups.ts`.)_ Puis T5 port/franco, T6 poids, T7 point d'entrée, T8/T9.
